...
train_data = torch.load("train_depressive_representations.pt")
test_data = torch.load("test_depressive_representations.pt")
...
model = DepressionClassifier(config).to(device)
...
model_path = "depression_classifier.pt"
...
"""LOADING THE MODEL"""
model = DepressionClassifier(config).to(device)
model.load_state_dict(torch.load("anxiety_classifier.pt", map_location=device))
model.eval()
